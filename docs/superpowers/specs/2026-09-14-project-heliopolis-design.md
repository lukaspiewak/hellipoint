...
| **Render (z bramki Fazy 0)** | **terminator musi być czytelny jako GRANICA, nie jako gradient.** Kryterium akceptacji: gracz wskazuje komórkę i mówi „ta świeci, ta nie" — bez UI, bez najeżdżania kursorem. Pomiar przy identycznej fazie słońca pokazał, że przy gładkim `saturate(dot)` interpolowanym po kuli terminatora **nie widać wcale**. D1 stoi na tym, że gracz czyta granicę wzrokiem, więc render gładki unieważnia filar. **Uwaga: to nie jest sprzeczne z §5.1 — symulacja liczy produkcję ciągle, render prezentuje ją czytelnie.** Zrównanie warstw w którąkolwiek stronę zepsuje albo ekonomię, albo czytelność |
| ↳ *jak to osiągnąć — otwarte, Faza 2/4* | **Wymaganiem jest własność, nie technika.** Trzy pasma ze spike'a to najtańszy sposób, jaki dało się zrobić w prototypie, nie rekomendacja. Najprawdopodobniejsza droga: **cieniowanie per komórka** zamiast interpolacji po powierzchni — teren to płaskie wieloboki, a §4.3 i tak liczy światło z normalnej komórki, więc granica sama biegnie po krawędziach heksów, ostra i zgodna co do komórki z tym, co liczy symulacja. Alternatywy warte sprawdzenia w Fazie 4: stroma `smoothstep` wokół terminatora, wyraźne obrzeże na samej granicy, albo **różnica temperatury barwowej** zamiast jasności — kontrast barwy czyta się szybciej i nie wymaga posteryzacji |
...
| ~~Q2~~ | ~~Kadencja ataku wroga~~ → **ROZSTRZYGNIĘTE: obrażenia ciągłe**, `dps × TICK_SECONDS` co tick, bez licznika zamachów. Przy 20 Hz nieodróżnialne od szybkich zamachów, a oszczędza stan na jednostkę — czyli mniej do serializowania w Fazie 5. Zachowanie przy kontakcie: jednostka atakuje budynek w swojej komórce, a gdy go nie ma — ten, który blokuje jej następny krok; ruch zatrzymuje ją przed zabudową, walka ją przegryza (`combat.ts`, `movement.ts`) | 1 ✅ |
